...
# #
def plot_prediction(image, boxes, labels, ax=None, plot_prob=True):
    bboxes_scaled = rescale_bboxes(boxes, list(image.shape[2:])[::-1])
    if ax is None:
        ax = plt.gca()
    plot_results(image[0].permute(1, 2, 0).detach().cpu().numpy(), bboxes_scaled, labels, ax, plot_prob=plot_prob)


def plot_results(pil_img, boxes, labels, ax, plot_prob=True, norm=True):
    from matplotlib import pyplot as plt
    image = plot_image(ax, pil_img, norm)

    # colors = COLORS * 100
    boxes_list = [sublist for sublist in boxes.tolist()[0]]
    # print(boxes_list)
    colors = COLORS[:len(boxes_list)]

    if boxes is not None:
        for cl, (xmin, ymin, xmax, ymax), c in zip(labels, boxes_list, colors):
            ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                       fill=False, color=c, linewidth=2))
...
def plot_prediction_GT(image, boxes, scores, labels, ax=None, plot_prob=True):
    bboxes_scaled = rescale_bboxes(boxes, list(image.shape[2:])[::-1])
    if ax is None:
        ax = plt.gca()
    plot_results_GT(image[0].permute(1, 2, 0).detach().cpu().numpy(), bboxes_scaled, scores, labels, ax, plot_prob)


def plot_results_GT(pil_img, boxes, scores, labels, ax, plot_prob=True, norm=True):
    from matplotlib import pyplot as plt
    image = plot_image(ax, pil_img, norm)
    # colors = COLORS * 100
    ...
    if boxes is not None:
        for sc, cl, (xmin, ymin, xmax, ymax), c in zip(scores, labels, boxes_list, colors):
            ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                       fill=False, color=c, linewidth=2))
            # print(cl)
            # print(sc)
            max_index_scalar = cl.item()
            max_value_scalar = sc.item()
            text = f'{CLASSES[max_index_scalar]}: {max_value_scalar:0.2f}'
            ax.text(xmin, ymin, text, fontsize=5, bbox=dict(facecolor='yellow', alpha=0.5))
    ax.grid('off')
